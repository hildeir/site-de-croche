export default class Carrossel{
    constructor(){
        this.root = document.querySelector(".conf");
        this.root_computed = window.getComputedStyle(this.root);
        this.property_width = this.root_computed.getPropertyValue("--width-carrossel");
        this.width_carrossel = parseInt(this.property_width.replace("px",""));
        this.total_items = document.querySelectorAll(".carrossel-item").length;
        this.carrossel_window = document.querySelector(".carrossel-window");
        ...
        this.item = document.querySelector(".carrossel-item");
    ...
    start(){
        const self = this;
        const state_animation = document.querySelector(".carrossel-window");
    ...
    updateLegenda(current_item){
        const total_items = document.querySelectorAll(".item-legenda");
        for (let i = 0; i < total_items.length; i++) {
            const element = total_items[i];
            element.classList.remove("active-item-legenda");
        }
        total_items[current_item].classList.add("active-item-legenda");
    }
    buildLegenda(){
        const legenda = document.querySelector(".legenda");
        const total_items = document.querySelectorAll(".carrossel-item");
        for (let i = 0; i < total_items.length; i++) {
            let div = document.createElement("div");
            div.classList.add("item-legenda");
